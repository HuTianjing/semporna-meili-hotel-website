...
const META_DIR = path.join(process.cwd(), 'planning', 'meta');

function getDocContent(slug: string): string | null {
  const decoded = decodeURIComponent(slug);
  for (const ext of ['.mdx', '.md']) {
    const fp = path.join(META_DIR, decoded + ext);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${getDocTitle(content, decodeURIComponent(slug))} — 美丽度假酒店策划`,
    robots: 'noindex, nofollow',
  };
}

export default async function MetaDocPage({ params }: Props) {
  const { slug } = await params;
  const content = getDocContent(slug);
  if (!content) notFound();
    ...
    <div className="max-w-4xl mx-auto px-6 py-8 pb-20">
      ...
      <article className="bg-white rounded-lg border border-slate-200 px-8 py-10 sm:px-12">
        <div
          ...
          prose-blockquote:border-l-4 prose-blockquote:border-indigo-400 prose-blockquote:bg-indigo-50 prose-blockquote:py-1 prose-blockquote:not-italic prose-blockquote:text-slate-700
      ...
      <div className="flex justify-between mt-6 gap-4">
        {prevSlug ? (
          <Link
            href={`/planning/meta/${encodeURIComponent(prevSlug)}`}
        ...
        {nextSlug ? (
          <Link
            href={`/planning/meta/${encodeURIComponent(nextSlug)}`}
