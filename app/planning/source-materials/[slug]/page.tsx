...
const SOURCE_DIR = path.join(process.cwd(), 'planning', 'source-materials');

function getDocContent(slug: string): string | null {
  const decoded = decodeURIComponent(slug);
  const candidates = [`${decoded}.md`, `${decoded}.mdx`];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${getDocTitle(content, decodeURIComponent(slug))} — 原始素材`,
    robots: 'noindex, nofollow',
  };
}

export default async function SourceMaterialPage({ params }: Props) {
  const { slug } = await params;
  const content = getDocContent(slug);
  if (!content) notFound();
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 pb-20">
      ...
      <nav className="flex items-center gap-2 mb-8 text-sm text-slate-500">
        ...
        <span>/</span>
        <span className="text-amber-600 font-medium">原始素材</span>
        <span>/</span>
        <span className="text-slate-800 truncate">{title}</span>
      </nav>

      {/* MDX Content */}
      <article className="bg-white rounded-lg border border-amber-200 px-8 py-10 sm:px-12">
        <div className="prose prose-slate prose-base max-w-none
          ...
          prose-blockquote:border-l-4 prose-blockquote:border-amber-400 prose-blockquote:bg-amber-50 prose-blockquote:py-1 prose-blockquote:not-italic prose-blockquote:text-slate-700
      ...
      <div className="flex justify-between mt-6 gap-4">
        {prevSlug ? (
          <Link
            href={`/planning/source-materials/${encodeURIComponent(prevSlug)}`}
        ...
        {nextSlug ? (
          <Link
            href={`/planning/source-materials/${encodeURIComponent(nextSlug)}`}
            className="inline-block px-4 py-2.5 bg-amber-600 rounded-md text-sm text-white font-medium no-underline hover:bg-amber-700 transition-colors"
          >
            {nextSlug} →
          </Link>
        ) : (
